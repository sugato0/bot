...
async def choose(message: types.Message) -> None:
    await message.reply(text="Выберите тип регистарции", 
...
async def cancle(message: types.Message, state: FSMContext) -> None:
    ...
    await message.reply('Отмена действий', reply_markup=kb_user.get_keyboard())
    await state.finish()
...
async def get_education(message: types.Message, state: FSMContext) -> None:
    ...
    await ClientStates.next()
    await message.reply('Отправьте логотип команды', reply_markup=kb_user.cancle_keyboard())
...
async def get_profession(message: types.Message, state: FSMContext) -> None:
    ...
    await ClientStates.next()
    await message.reply('Отправьте фотографию вашего студентческого билета', reply_markup=kb_user.cancle_keyboard())
...
def register_handlers_user(dp: dispatcher):
    ...
    dp.register_message_handler(cancle, Text(equals='Отмена'), state='*')
